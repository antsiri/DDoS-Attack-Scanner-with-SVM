...
class SimpleMonitor(SimpleSwitch13):
    ...
    def _flow_stats_reply_handleer(self, ev):
        body = ev.msg.body
        ...
        for stat in sorted([flow for flow in body if flow.priority >= 1], key=lambda flow: (flow.instructions[0].actions[0])):
            ...
            if(int(stat.byte_count) > 2000000):
                ...
                mod = parser.OFPlowMod(datapath=dp, priority=2, match=match, instructions=inst)
                dp.send_msg(mod)
